...
for i in range(len(file_names)):
    try:
        ...
        output(data, beta, truth, t).to_csv('../../Simulation_data_GLMM/Result_GH/Setting_5_' +
...
for i in range(len(file_names)):
    try:
        ...
        [beta, mu] = GH(k, data, out)
        t = time.time() - start_time
        output(data, beta, truth, t).to_csv('../../Simulation_data_GLMM/Result_GH/Setting_6_' +
                                              file_names[i][43:], header = True)
        print('======\n File:\n', file_names[i], '\n Completed!\n')
        print("--- %s seconds ---" % (time.time() - start_time))
    except:
...
#         Load data
        ...
        [beta, mu] = GH(k, data, out)
        t = time.time() - start_time
        output(data, beta, truth, t).to_csv('../../Simulation_data_GLMM/Result_GH/Setting_7_' +
...
#         Load data
        ...
        [beta, mu] = GH(k, data, out)
        t = time.time() - start_time
        output(data, beta, truth, t).to_csv('../../Simulation_data_GLMM/Result_GH/Setting_8_' +
                                              file_names[i][44:], header = True)
        print('======\n File:\n', file_names[i], '\n Completed!\n')
        print("--- %s seconds ---" % (time.time() - start_time))
    except:
