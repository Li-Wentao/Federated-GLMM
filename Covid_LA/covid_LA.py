...
# Regression
[beta, mu, lam, score] = LA(data, out)
t = time.time() - start_time
output(data, beta, t, var_name).to_csv('../../Covid_data_GLMM/Result_LA/result_la.csv')
aic = -2 * score + 2 * len(beta)
bic = -2 * score + np.log(46312) * len(beta)
with open('../../Covid_data_GLMM/Result_LA/la_stats.txt', 'w') as f:
    f.write('Mu: \n' + str(mu))
    f.write('\n\nLambda: \n' + str(lam))
    f.write('\n\nLog-likelihood: \n' + str(score))
    f.write('\n\nAIC: \n' + str(aic))
